...
<?php else : ?>
<?php
$to = $webweb_wp_digishop_obj->get('plugin_support_email');
...
$site = app_contact_get_var('site', $webweb_wp_digishop_obj->get('site_url'));
...
if (!empty($_POST)) {
    ...
    if (empty($errors)) {
        ...
        if ($status) {
            $contact_msg = $webweb_wp_digishop_obj->message("Your message has been sent.", 1);
            // everything went fine let's clear the fields.
            $name = $phone = $email = $site = $subject = $message = '';
        } else {
            $contact_msg = $webweb_wp_digishop_obj->message("Cannot send email. Please send your request directly to: $to");
        }
    } else {
        $contact_msg = $webweb_wp_digishop_obj->message("<h3>Errors: </h3>" . join("<br/>\n", $errors));
    }
}
